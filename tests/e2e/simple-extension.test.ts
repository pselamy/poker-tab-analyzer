...
describe("Poker Tab Analyzer Extension - Simple Tests", () => {
  ...
  describe("Extension Files", () => {
    it("should have all required extension files", () => {
      ...
      expect(fs.existsSync(path.join(extensionPath, "manifest.json"))).toBe(
        true,
      );
      expect(fs.existsSync(path.join(extensionPath, "background.js"))).toBe(
        true,
      );
  ...
  describe("Test Page", () => {
    ...
    it("should change hands when buttons are clicked", async () => {
      ...
      const communityCards = await page.$$eval(
        ".community-cards .card",
        (cards) => cards.map((card) => card.getAttribute("data-card")),
      );
