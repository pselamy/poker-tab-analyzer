...
function isPokerSite(url) {
  ...
  const pokerDomains = [
    'poker', 'globalpoker', 'pokerstars', 'ggpoker', 
    'partypoker', 'wsop', '888poker', 'americascardroom'
  ];
...
function updateStats(state) {
  if (state.detections) {
    ...
    const detectedCount = recentDetections.filter(d => d.cards.length > 0).length;
    const rate = recentDetections.length > 0 
